...
def p_relational(self: Parser) -> Expression:
    ...
    equivalent EBNF:
    rational: additive { '<' additive | '>' additive | '<=' additive | '>=' additive }
    """
    lookahead = self.lookahead
...
def p_assignment(self: Parser) -> Expression:
    ...
    if self.next == "Assign":
        ...
        node = Assignment(node, rhs) ## 这里是否调用产生式cond->lor->lad->eq->rela->add->multi->unary->primary->integer，可看出本文法不为LL（1）
        return node
    else:
        return node
...
def p_if(self: Parser) -> If:
    ...
    expr = p_expression(self)
    lookahead("RParen")
    stmt = p_statement(self)
    if_n = If(expr, stmt)
    if self.next == "Else":
        lookahead("Else")
        if_n.otherwise = p_statement(self)
    return if_n
